# Read-back test for the tabulated table UserObjects produced by
# ve_upscale_curves.py. The [UserObjects] block (relperm_uo = VERelPermTableUO,
# pc_uo = VECapillaryPressureTableUO) is the script's emitted curves_upscaled.i,
# !include'd verbatim -- so this also proves the script output is in the format the
# UOs accept. The script-regression test (prereq) regenerates curves_upscaled.i.
#
# No flow solve: sat_n is driven to 0.1, 0.2, ..., 0.6 (one per timestep) and the
# UOs are sampled there:
